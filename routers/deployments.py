...
def create_kube_namespace(name: str):
    ...
    body = client.V1Namespace(metadata=client.V1ObjectMeta(name="jupyterhub-" + name))
    try:
...
def create_services(namespace=DeploymentType.dummy):
    ...
    spec.ports = [client.V1ServicePort(protocol="TCP", port=8001, target_port=8001)]
    service.spec = spec
...
def create_configmap(namespace: str):
    ...
    with open("yamls/hub/configmap.yaml") as f:
        dep = yaml.safe_load(f)
        resp = k8s_core_v1.create_namespaced_config_map(body=dep, namespace=namespace)


def create_pvc(namespace: str):
    with open("yamls/hub/pvc.yaml") as f:
        dep = yaml.safe_load(f)
        resp = k8s_core_v1.create_namespaced_persistent_volume_claim(
...
def create_rbac(namespace: str):
    ...
    serviceaccount.metadata = client.V1ObjectMeta(
        name="hub", labels={"component": "jupyter"}
    )

    k8s_core_v1.create_namespaced_service_account(
        namespace=namespace, body=serviceaccount
    )
    ...
    resp = api_instance.create_namespaced_role(namespace=namespace, body=role)
    ...
    subjects = client.V1Subject(kind="ServiceAccount", name="hub")
    roleref = client.V1RoleRef(
        api_group="rbac.authorization.k8s.io", kind="Role", name="hub"
    )
    rolebinding = client.V1RoleBinding(
        metadata=metadata, subjects=[subjects], role_ref=roleref
    )
    resp = api_instance.create_namespaced_role_binding(
        namespace=namespace, body=rolebinding
    )
